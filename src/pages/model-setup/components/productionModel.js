...
const ProductionModel = ({
  ...
  const intiDataForPreInpage = async () => {
    ...
    if (
      checkPreviousDataset &&
      checkPreviousDataset != undefined &&
      checkPreviousDataset != null
    ) {
      let fileSelectedPath =
        checkPreviousDataset.data.body[0].ProdFile != null
          ? checkPreviousDataset.data.body[0].ProdFile
          : [];
      if (fileSelectedPath.length > 0) {
        setDBFilePath(fileSelectedPath);
        const splitFilePath = fileSelectedPath.split(",");
  ...
  const saveHandler = async () => {
    ...
    const savedData = await updateModelProductionDataApi(saveTrainData);
    let message =
  ...
  return (
    <React.Fragment>
      ...
      <p>
        Select the script to be run in production. This can be an R script a
        python script or a Jupyter notebook file.
      </p>
      <Row>
        <Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(ProductionModel);
